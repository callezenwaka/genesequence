...
class Sequence:
    ...
    def generate_sequence(self, length=10, sequence_type="DNA"):
        ...
        sequence = ''.join([random.choice(NUCLEOTIDE_BASE[sequence_type])
    ...
    def gen_reading_frames(self):
        ...
        tmp_seq = Sequence(self.reverse_complement().split(" ")
                           [-1], self.sequence_type)
    ...
    def all_proteins_from_orfs(self, startReadPos=0, endReadPos=0, ordered=False):
        ...
        if endReadPos > startReadPos:
            tmp_seq = Sequence(
                self.sequence[startReadPos: endReadPos], self.sequence_type)
